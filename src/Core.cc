...
void Core::issue(std::unique_ptr<Tile> op) {
    ...
    _spad.flush(spad_id);
    if (!op->accum ||
        !(_current_layer_id == op->layer_id && _current_fused_op_id == op->fused_op_id)) {
...
void Core::cycle() {
    ...
    for (int tile_iter = 0; tile_iter < _tiles.size(); tile_iter++) {
        ...
        if (inst->opcode == Opcode::MOVIN) {
            /*LD inst queue */
            if (inst->size == 0) {
                spdlog::error("[Core {}] MVIN issue addr: {:x}, size: {:x}",
                              _id,
                              inst->dest_addr,
                              inst->size);
            }
            if (!buffer->check_allocated(inst->dest_addr, buffer_id) &&
                ...
                /*Invalid state */
                spdlog::error("Destination allocated: {} Size remain: {}",
                              buffer->check_allocated(inst->dest_addr, buffer_id),
                              buffer->check_remain(inst->size, buffer_id));
                spdlog::error("[Core {}] MVIN issue panic addr: {:x}, size: {} B",
                              _id,
                              inst->dest_addr,
                              inst->size * _config.dram_req_size);
    ...
    for (auto tile = _tiles.begin(); tile < _tiles.end(); tile++) {
        if ((*tile)->instructions.empty() && (*tile)->inst_finished) {
            ...
            (*tile)->stat.memory_stall = (*tile)->stat.cycles - (*tile)->stat.compute_cycles;
...
bool Core::can_issue_compute(std::unique_ptr<Instruction>& inst) {
    ...
    if (!result) {
        for (addr_type addr : inst->src_addrs) {
            spdlog::trace("Core[{}] Dependency fail : {} , {}",
                          _id,
                          addr,
...
void Core::print_stats() {
    update_stats();
    spdlog::info("Core [{}] : MatMul active cycle {} Vector active cycle {} ",
                 _id,
                 _stat_tot_matmul_cycle,
                 _stat_tot_vec_compute_cycle);

    spdlog::info(
            "Core [{}] : Memory unit idle cycle {} Systolic bubble cycle {} "
            "Core idle cycle {} ",
            _id,
            _stat_tot_memory_idle_cycle,
            _stat_tot_systolic_bubble_cycle,
            _stat_tot_idle_cycle);

    spdlog::info(
            "Core [{}] : Systolic Array Utilization(%) {:.2f} ({:.2f}% PE util), Vector Unit "
            "Utilization(%) {:.2f}, Total cycle: {}",
            _id,
            static_cast<float>(_stat_tot_systolic_active_cycle * 100) / _core_cycle,
...
void Core::print_current_stats() {
    ...
    spdlog::log(level, "Core [{}] : issued tile {} ", _id, _tiles.size());
...
void Core::finish_compute_pipeline() {
    if (!_compute_pipeline.empty() && _compute_pipeline.front()->finish_cycle <= _core_cycle) {
        ...
        double compute_size =
                inst->tile_k * inst->tile_m * inst->tile_n /
                (_config.core_config[_id].core_height * _config.core_config[_id].core_width);
        spdlog::trace("Compute size {} tile m {} tile k {} tile n {}",
                      inst->compute_size,
                      inst->tile_m,
                      inst->tile_k,
                      inst->tile_n);
        spdlog::trace("Compute size {} , compute time {}",
                      compute_size,
...
void Core::finish_vector_pipeline() {
    if (!_vector_pipeline.empty() && _vector_pipeline.front()->finish_cycle <= _core_cycle) {
...
void Core::handle_ld_inst_queue() {
    if (!_ld_inst_queue.empty()) {
        ...
        if (front->opcode == Opcode::MOVIN) {
            ...
            if (front->size == 0) {
                spdlog::error("Destination size is 0! opcode: {}, addr: 0x{:x}",
                              (int)front->opcode,
                              front->dest_addr);
            }
            ...
            if (!ret) {
                spdlog::error("Destination allocated: {} Size remain: {}",
                              buffer->check_allocated(front->dest_addr, buffer_id),
                              buffer->check_remain(front->size, buffer_id));
                spdlog::error("instruction panic opcode: {:x}, addr: {:x}, size: {} B",
                              (int)front->opcode,
                              front->dest_addr,
            ...
            for (addr_type addr : front->src_addrs) {
                ...
                MemoryAccess* access = new MemoryAccess({.id = generate_mem_access_id(),
