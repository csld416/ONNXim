...
uint32_t Dram::get_channel_id(MemoryAccess* access) {
    uint32_t channel_id;
    if (_n_ch >= 16)
        channel_id = ipoly_hash_function(
                (new_addr_type)access->dram_address / _config.dram_req_size, 0, _n_ch);
    else
        channel_id = ipoly_hash_function(
                             (new_addr_type)access->dram_address / _config.dram_req_size, 0, 16) %
                     _n_ch;
    return channel_id;
}
...
void SimpleDram::cycle() {
    for (uint32_t ch = 0; ch < _n_ch; ch++) {
        if (!_waiting_queue[ch].empty() && _waiting_queue[ch].front().first <= _cycles) {
...
bool SimpleDram::is_full(uint32_t cid, MemoryAccess* request) {
    return false;
}
...
DramRamulator::DramRamulator(SimulationConfig config)
    : _mem(std::make_unique<ram::Ramulator>(config.dram_config_path, config.num_cores, false)) {
    _n_ch = config.dram_channels;
    _config = config;
    _cycles = 0;
