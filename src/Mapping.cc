...
std::string Mapping::to_string() {
    std::stringstream ss;
    ss << "Total Loop: N" << total_loop.N << " C" << total_loop.C << " M" << total_loop.M << " S"
       << total_loop.S << " R" << total_loop.R << " Q" << total_loop.Q << " P" << total_loop.P
       << " Tile Out Loop: N" << tile_out_loop.N << " C" << tile_out_loop.C << " M"
       << tile_out_loop.M << " S" << tile_out_loop.S << " R" << tile_out_loop.R << " Q"
       << tile_out_loop.Q << " P" << tile_out_loop.P << " Tile In Loop: N" << tile_in_loop.N << " C"
       << tile_in_loop.C << " M" << tile_in_loop.M << " S" << tile_in_loop.S << " R"
       << tile_in_loop.R << " Q" << tile_in_loop.Q << " P" << tile_in_loop.P;
    return ss.str();
}
...
MappingTable MappingTable::parse_mapping_file(std::string mapping_path, SimulationConfig config) {
...
void MappingTable::gemm_mapping(Mapping::LoopCounts& key) {
    ...
    uint32_t max_spad_rows =
            (_config.core_config[key.target_core].spad_size KB) / (dim * _config.precision * 2);
    uint32_t max_acc_rows =
            (_config.core_config[key.target_core].accum_spad_size KB) / (dim * 4 * 2);

    assert(_config.core_config[key.target_core].core_height ==
           _config.core_config[key.target_core].core_width);
    dim_I = key.N;
    dim_J = key.M;
    dim_K = key.C;
    ...
    spdlog::info("[GEMM] spad_size: {} accum_size: {}",
                 _config.core_config[key.target_core].spad_size * 1024,
                 _config.core_config[key.target_core].accum_spad_size * 1024);
    spdlog::info("[GEMM] required_sram_size: {} required_accum_size: {}",
                 (inner_I + inner_J) * inner_K * _config.precision,
                 (inner_I * inner_J) * _config.precision);
    spdlog::info(
            "[GEMM] Used gemmini gemm mapping: Total N:{} C:{} M:{}, "
            "Outer N:{} C:{} M:{}, "
            "Inner N:{} C:{} M:{}",
            mapping.total_loop.N,
            mapping.total_loop.C,
            mapping.total_loop.M,
            mapping.tile_out_loop.N,
            mapping.tile_out_loop.C,
            mapping.tile_out_loop.M,
            mapping.tile_in_loop.N,
            mapping.tile_in_loop.C,
            mapping.tile_in_loop.M);
}
...
int MappingTable::_calc_conv_mapping(bool acc,
    ...
    const int A_rows =
            trans_input_3120
                    ...
                    : (in_channels_per_bank * batches * (irows >> downsample) *
                       (icols >> downsample));

    const int B_rows = trans_weight_0132 ? in_channels_per_bank * kcols * krows * ochs
...
Mapping MappingTable::calc_conv_mapping(Mapping::LoopCounts& key) {
    ...
    while (!nothing_increased) {
        ...
        for (size_t i = 0; i < sizeof(args) / sizeof(args[0]); i++) {
            ...
            int weight_tile_size =
                    args[0] * args[3] * args[4] * args[5] * args[6] * _config.precision;
            // B * O_row * O_col * O_ch
            int input_tile_size = args[0] * (args[1] + 2 * padding) * (args[2] + 2 * padding) *
                                  args[6] * _config.precision;
            if (spad_rows <= max_spad_rows && acc_rows <= max_acc_rows &&
                ((input_tile_size + weight_tile_size) * 3 >> 1) <=
                        (_config.core_config[key.target_core].spad_size KB / 2)) {
    ...
    batch_size = key.N;
    Mapping mapping;
    mapping.total_loop = {(uint32_t)batch_size,
                          ...
                          (uint32_t)out_dim,
                          (uint32_t)out_dim};
    mapping.tile_out_loop = {ceil_div(batch_size, batches),
                             ceil_div(in_channels, kchs),
                             ceil_div(out_channels, ochs),
                             ceil_div(kernel_dim, krows),
                             ceil_div(kernel_dim, kcols),
                             ceil_div(out_dim, ocols),
                             ceil_div(out_dim, orows)};
    mapping.tile_in_loop = {(uint32_t)batches,
                            (uint32_t)kchs,
                            (uint32_t)ochs,
                            (uint32_t)krows,
                            (uint32_t)kcols,
                            ...
                            (uint32_t)orows};
    spdlog::info(
            "[Conv] Used gemmini convolution mapping: "
            "[T] N{} C{} M{} P{} Q{} S{} R{} - "
            "[O] N{} C{} M{} P{} Q{} S{} R{} - "
            "[I] N{} C{} M{} P{} Q{} S{} R{}",
            batch_size,
