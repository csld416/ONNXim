...
Model::Model(std::string onnx_path,
             json model_config,
    ...
    if (_model_config.contains("request_time"))
        _request_time = uint64_t(double(_model_config["request_time"]) * 1000 * 1000 *
                                 1000);  // Pico seconds
    else
        _request_time = 0;
...
void Model::initialize_model(std::vector<std::unique_ptr<Tensor>>& weight_table) {
    ...
    for (auto iter : input) {
        ...
        auto input_tensor = std::make_unique<Tensor>(
                _root_node_id, input_name, input_dim, _config.precision * 16, true);
    ...
    for (auto node_proto : model_proto.graph().node()) {
        ...
        if (node != nullptr) {
            ...
            if (node_proto.op_type() == "SkipLayerNormalization" &&
                _model_config.contains("nr_atten"))
                if (_model_config["nr_atten"] != -1 &&
                    ++nr_skip >= int(_model_config["nr_atten"]) * 2) {
    ...
    for (auto& [key, val] : _operation_map) {
        ...
        if (val->get_optype() == "Attention") {
            ...
            _operation_map[projection_id] =
                    std::move(std::unique_ptr<GemmWS>(attention_node->_projection_node));
