...
Simulator::Simulator(SimulationConfig config, bool language_mode)
    ...
    for (int i = 0; i < config.num_cores; i++)
        spdlog::info(
                "[Core {}] Systolic Array Throughput: {} GFLOPS, Spad size: {} KB, Accumulator "
                "size: {} KB",
                i,
                config.max_systolic_flops(i),
                config.core_config[i].spad_size,
                config.core_config[i].accum_spad_size);
    ...
    if (config.dram_type == DramType::SIMPLE) {
        ...
        std::string ramulator_config =
                fs::path(onnxim_path).append("configs").append(config.dram_config_path).string();
...
void Simulator::cycle() {
    ...
    while (running()) {
        ...
        if (_cycle_mask & ICNT_MASK) {
            ...
            for (int mem_id = 0; mem_id < _n_memories; mem_id++) {
                ...
                if (!_dram->is_empty(mem_id) &&
                    ...
                    _icnt->push(core_offset + mem_id,
                                get_dest_node(_dram->top(mem_id)),
                                _dram->top(mem_id));
                    _dram->pop(mem_id);
                    _nr_from_mem++;
                }
            ...
            if (_icnt_interval != 0 && _icnt_cycle % _icnt_interval == 0) {
                spdlog::info("[ICNT] Core->ICNT request {}GB/Sec",
                             ((_memory_req_size * _nr_from_core * (1000 / _icnt_period) /
                               _icnt_interval)));
                spdlog::info("[ICNT] Core<-ICNT request {}GB/Sec",
                             ((_memory_req_size * _nr_to_core * (1000 / _icnt_period) /
                               _icnt_interval)));
                spdlog::info(
                        "[ICNT] ICNT->MEM request {}GB/Sec",
                        ((_memory_req_size * _nr_to_mem * (1000 / _icnt_period) / _icnt_interval)));
                spdlog::info("[ICNT] ICNT<-MEM request {}GB/Sec",
                             ((_memory_req_size * _nr_from_mem * (1000 / _icnt_period) /
                               _icnt_interval)));
                _nr_from_core = 0;
                _nr_to_core = 0;
                _nr_to_mem = 0;
    ...
    spdlog::info("Simulation Finished at {} cycle {} us",
                 _core_cycles,
                 _core_cycles / (_config.core_freq));
...
uint32_t Simulator::get_dest_node(MemoryAccess* access) {
    if (access->request) {
        return _config.num_cores * _config.icnt_injection_ports_per_core +
               _dram->get_channel_id(access);
    } else {
        return access->core_id * _config.icnt_injection_ports_per_core +
               (_dram->get_channel_id(access) % _config.icnt_injection_ports_per_core);
    }
}
