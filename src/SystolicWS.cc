...
void SystolicWS::cycle() {
  ...
  */
    ...
    if (!_ex_inst_queue.empty() &&
        can_issue_compute(_ex_inst_queue.front())) {  // execution dependecy check
        ...
        if (front->dest_addr >= ACCUM_SPAD_BASE) {
            if (_acc_spad.check_allocated(front->dest_addr, front->accum_spad_id)) {
                ...
                int ret = _acc_spad.prefetch(front->dest_addr,
                                             front->accum_spad_id,
                                             front->size,
                                             front->zero_init ? front->size : 1);
                if (!ret) {
                    spdlog::error("Destination allocated: {} Size remain: {}",
                                  _acc_spad.check_allocated(front->dest_addr, front->accum_spad_id),
                                  _acc_spad.check_remain(front->size, front->accum_spad_id));
            ...
            if (_spad.check_allocated(front->dest_addr, front->spad_id)) {
                ...
                int ret = _spad.prefetch(front->dest_addr,
                                         front->spad_id,
                                         front->size,
                                         front->zero_init ? front->size : 1);
                if (!ret) {
                    spdlog::error("Destination allocated: {} Size remain: {}",
                                  _spad.check_allocated(front->dest_addr, front->spad_id),
                                  _spad.check_remain(front->size, front->spad_id));
                    spdlog::error("instruction panic opcode: {:x}, addr: {:x}, size: {} B",
                                  (int)front->opcode,
                                  front->dest_addr,
                                  front->size * _config.dram_req_size);
            ...
            if (!_compute_pipeline.empty()) {
                ...
                if (_compute_pipeline.back()->start_cycle + offset < _core_cycle) {
                    ...
                    _stat_systolic_bubble_cycle +=
                            (_core_cycle - _compute_pipeline.back()->start_cycle + offset);
                } else
                ...
                if (front->opcode == Opcode::GEMM_PRELOAD) {
                    ...
                    front->start_cycle += _config.core_config[_id].core_height +
                                          _config.core_config[_id].core_height - 1;
                    _stat_systolic_preload_issue_count++;
                }
            }
...
cycle_type SystolicWS::get_inst_compute_cycles(std::unique_ptr<Instruction>& inst) {
    return _config.core_config[_id].core_height + _config.core_config[_id].core_width - 2 +
           MAX(inst->compute_size, 4);
}
...
cycle_type SystolicWS::get_vector_compute_cycles(std::unique_ptr<Instruction>& inst) {
    ...
    switch (inst->opcode) {
        ...
        case Opcode::SOFTMAX:
            ...
            vector_ops = vec_op_iter * (_config.core_config[_id].add_latency +
                                        _config.core_config[_id].exp_latency +
                                        _config.core_config[_id].mul_latency);
...
void SystolicWS::print_stats() {
    Core::print_stats();
    spdlog::info(
            "Core [{}] : Systolic Inst Issue Count : {}", _id, _stat_systolic_inst_issue_count);
    spdlog::info("Core [{}] : Systolic PRELOAD Issue Count : {}",
                 _id,
                 _stat_systolic_preload_issue_count);
}