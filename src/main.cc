...
int main(int argc, char** argv) {
    ...
    for (json model_config : models_list["models"]) {
        if (language_mode) {
            ...
            std::string mapping_path =
                    fmt::format("{}/{}/{}.mapping", model_base_path, model_name, model_name);
            MappingTable mapping_table = MappingTable::parse_mapping_file(mapping_path, config);

            auto model = std::make_unique<Model>(
                    onnx_path, model_config, config, model_name, mapping_table);
    ...
    spdlog::info("Total tile: {}, simulated tile per seconds(TPS): {:3f}",
                 simulator->get_number_tile(),
                 simulator->get_tile_ops());
    return 0;
}
