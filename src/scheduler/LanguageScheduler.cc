...
std::unique_ptr<LangScheduler> LangScheduler::create(std::string name,
                                                     std::string path,
    ...
    if (info["scheduler"] == "simple") {
        ...
        return std::make_unique<LangScheduler>(
                name, path, std::move(model), config, info["scheduler_config"]);
    } else if (info["scheduler"] == "iter") {
        spdlog::info("Iter Language scheduler selected");
        return std::make_unique<IterLevelScheduler>(
                name, path, std::move(model), config, info["scheduler_config"]);
    } else {
...
LangScheduler::LangScheduler(std::string name,
                             std::string path,
                             std::unique_ptr<LanguageModel> model,
                             SimulationConfig config,
    ...
    if (_active_requests.empty()) {
        while (!_request_queue.empty()) {
            if (_request_queue.front()->request_time <= _cycle) {
                ...
                _active_requests[_request_queue.front()->request_id] =
                        std::move(_request_queue.front());
                _request_queue.pop();
            } else {
                break;
            }
...
uint64_t LangScheduler::get_kv_memory_size() {
    ...
    for (auto iter = _active_requests.begin(); iter != _active_requests.end(); iter++) {
        for (uint32_t i = 0; i < _num_sim_layers; i++) {
            kv_size += iter->second->key_cache[i]->get_size() +
                       iter->second->value_cache[i]->get_size();
        }
    }
...
void LangScheduler::init_request(std::unique_ptr<LangRequest>& request) {
    ...
    for (uint32_t i = 0; i < _num_sim_layers; i++) {
        ...
        request->key_cache[i] = std::make_unique<Tensor>(_language_model->get_root_node_id(),
                                                         name_gen(LAYER(i), "KeyCache"),
                                                         _max_dims,
                                                         _config.precision,
                                                         true);
        request->key_cache[i]->resize_tensor(first_dims);
        request->value_cache[i] = std::make_unique<Tensor>(_language_model->get_root_node_id(),
                                                           name_gen(LAYER(i), "ValueCache"),
                                                           _max_dims,
                                                           _config.precision,
                                                           true);
...
void LangScheduler::init_inputs_and_model() {
    ...
    if (!inputs.empty()) {
        ...
        if (_config.dram_size < tot_mem && _config.dram_size > 0) {
            if (_check_mem_size) {
                spdlog::error("Memory Usage exceeds the memory size limit {} GB/{} GB",
                              tot_mem,
                              _config.dram_size);
                exit(EXIT_FAILURE);
            } else {
                spdlog::warn("Memory Usage exceeds the memory size limit {} GB/{} GB",
                             tot_mem,
                             _config.dram_size);
            }
        }
