...
std::unique_ptr<Scheduler> Scheduler::create(SimulationConfig config,
    ...
    if (config.scheduler_type == "simple") {
        ...
        return std::make_unique<DedicatedCPUScheduler>(config, core_cycle, core_time, simulator);
    } else if (config.scheduler_type == "time_multiplex") {
        return std::make_unique<TimeMultiplexScheduler>(config, core_cycle, core_time, simulator);
...
Scheduler::Scheduler(SimulationConfig config,
                     const cycle_type* core_cycle,
                     const uint64_t* core_time,
                     void* simulator)
...
void Scheduler::schedule_model(std::unique_ptr<Model> model, uint32_t sample_size) {
    _request_queue.push_back(Request{
            .request_id = generate_id(), .model = std::move(model), .sample_size = sample_size});
    spdlog::info("MODEL {} Scheduled, Total Request: {}",
                 _request_queue.back().model->get_name(),
                 _request_queue.size());
    refresh_status();
}
...
std::unique_ptr<Tile> Scheduler::get_tile(uint32_t core_id) {
    ...
    if (_core_executable_tile_queue[core_id].empty() &&
        _executable_tile_queue[partition_id].empty()) {
        refresh_status();
        ...
        if (!_core_executable_tile_queue[core_id].empty()) {
            ...
            spdlog::debug("Layer {} Core {} Get Tile at {}",
                          _active_layers_map[tile->layer_id].name,
                          core_id,
                          *_core_cycle);
            return tile;
        } else {
...
void Scheduler::finish_tile(uint32_t core_id, int layer_id) {
    spdlog::debug("Layer {} Core {} Finish Tile at {} Remain tile {}",
                  layer_id,
                  core_id,
                  *_core_cycle,
                  _active_layers_map[layer_id].remain_tiles);
    ...
    if (_active_layers_map[layer_id].remain_tiles == 0) {
        ...
        spdlog::info("Layer {} finish at {}", _active_layers_map[layer_id].name, *_core_cycle);
...
void Scheduler::refresh_status() {
    if (!_request_queue.empty()) {
        if (_request_queue.front().model->check_finish()) {
            ...
            if (finished_model->check_language_model()) {
                static_cast<Simulator*>(_simulator)
                        ->finish_language_model(finished_model->get_id());
            }
    ...
    if (!_request_queue.empty() && all_empty && count_active_layers() == 0) {
        ...
        _executable_tile_queue[0].insert(_executable_tile_queue[0].end(),
...
void DedicatedCPUScheduler::refresh_status() {
    if (!_request_queue.empty()) {
        for (auto req = _request_queue.begin(); req != _request_queue.end(); req++) {
            if (req->model->check_finish()) {
                spdlog::info(
                        "Model[{}] Request: {} us, Start: {} us, finish:{} us Current Cycle:{}",
                        req->model->get_name(),
    ...
    if (!_request_queue.empty()) {
        for (int i = 0; i < _config.num_cores; i++) {
            ...
            if (_active_layers_map.find(new_layer->get_id()) == _active_layers_map.end()) {
                if (count_active_layers() > 0)
                    ...
                    spdlog::info(
                            "Layer {} {}: Enqueue", new_layer->get_name(), new_layer->get_id());
                ...
                spdlog::error(
                        "Accessed executing layer... id:{}, name:{} total:{} remain:{} fin:{} "
                        "launched:{}",
                        _active_layers_map[new_layer->get_id()].id,
                        _active_layers_map[new_layer->get_id()].name,
                        _active_layers_map[new_layer->get_id()].total_tiles,
                        _active_layers_map[new_layer->get_id()].remain_tiles,
                        _active_layers_map[new_layer->get_id()].finished_tiles,
                        _active_layers_map[new_layer->get_id()].launched_tiles);
            }
        }
    }
}
...
void TimeMultiplexScheduler::finish_tile(uint32_t core_id, int layer_id) {
    ...
    if (_active_layers_map[layer_id].remain_tiles == 0) {
        ...
        for (int req_index = 0; req_index < _request_queue.size(); req_index++) {
            if (_request_queue[req_index].request_id == _active_layers_map[layer_id].request_id) {
                model_finish = true;
        ...
        spdlog::info("Layer {} {} finish at {}",
                     model_name,
                     _active_layers_map[layer_id].name,
                     *_core_cycle);
        spdlog::info("Total compute time {}",
                     *_core_cycle - _active_layers_map[layer_id].start_cycle);
        assert(model_finish);
...
void TimeMultiplexScheduler::refresh_status() {
    ...
    if (!_request_queue.empty() && all_empty) {
        ...
        Operation* new_layer = _request_queue[_request_rr].model->get_executable_tile();
        /* Check executable layer exist */
        if (new_layer == nullptr)
            return;
        ...
        if (_active_layers_map.find(new_layer->get_id()) == _active_layers_map.end()) {
            if (count_active_layers() > 0)
                ...
                spdlog::info("Layer {} {}: Enqueue", new_layer->get_name(), new_layer->get_id());
...
void HalfSplitScheduler::schedule_model(std::unique_ptr<Model> model, uint32_t sample_size) {
    _request_queue.push_back(Request{
            .request_id = generate_id(), .model = std::move(model), .sample_size = sample_size});
    spdlog::info("MODEL {} Scheduled, Total Request: {}",
                 _request_queue.back().model->get_name(),
    ...
    if (_active_layers_map[layer_id].remain_tiles == 0) {
        ...
        for (int req_index = 0; req_index < _request_queue.size(); req_index++) {
            if (_request_queue[req_index].request_id == _active_layers_map[layer_id].request_id) {
                ...
                _request_queue[req_index].model->set_layer_finish(layer_id);
                model_name = _request_queue[req_index].model->get_name();
                _executable_tile_queue_table.erase(_request_queue[req_index].request_id);
            }
        }
        spdlog::info("Layer {} {} finish at {}",
                     model_name,
...
void HalfSplitScheduler::refresh_status() {
    if (!_request_queue.empty()) {
        for (auto req = _request_queue.begin(); req != _request_queue.end(); req++) {
            if (req->model->check_finish()) {
                spdlog::info("Model[{}] finish  at {}", req->model->get_name(), *_core_cycle);
                std::unique_ptr<Model> finished_model = std::move(req->model);
                req = _request_queue.erase(req);
                if (finished_model->check_regressive()) {
                    finished_model->prepare_regressive();
        ...
        for (auto req = _request_queue.begin(); req != _request_queue.end(); req++) {
            if (_executable_tile_queue_table[req->request_id].empty()) {
                ...
                if (_active_layers_map.find(new_layer->get_id()) == _active_layers_map.end()) {
                    ...
                    _executable_tile_queue_table[req->request_id].insert(
                            _executable_tile_queue_table[req->request_id].begin(),
                    ...
                    _active_layers_map[new_layer->get_id()] = LayerStat{
                            ...
                            .start_cycle = *_core_cycle,
                            .total_tiles =
                                    (uint32_t)_executable_tile_queue_table[req->request_id].size(),
                            .remain_tiles =
                                    (uint32_t)_executable_tile_queue_table[req->request_id].size(),
                            .finished_tiles = 0,
                            .launched_tiles = 0};
