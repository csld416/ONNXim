...
class Scheduler {
   public:
    static std::unique_ptr<Scheduler> create(SimulationConfig config,
    ...
    virtual void issue_tile_per_core(std::vector<uint32_t>& allowed_cpu,
                                     int offset,
...
class TimeMultiplexScheduler : public Scheduler {
   public:
    TimeMultiplexScheduler(SimulationConfig config,
                           const cycle_type* core_cycle,
                           const uint64_t* core_time,
                           void* simulator);
...
class DedicatedCPUScheduler : public TimeMultiplexScheduler {
   public:
    DedicatedCPUScheduler(SimulationConfig config,
                          const cycle_type* core_cycle,
                          const uint64_t* core_time,
                          void* simulator);
...
class HalfSplitScheduler : public Scheduler {
   public:
    HalfSplitScheduler(SimulationConfig config,
                       const cycle_type* core_cycle,
                       const uint64_t* core_time,
                       void* simulator);
   ...
   protected:
    virtual void refresh_status() override;
    robin_hood::unordered_map<uint32_t, std::deque<std::unique_ptr<Tile>>>
            _executable_tile_queue_table;
};
